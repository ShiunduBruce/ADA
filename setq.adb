...
package body Setq is
    ...
    function "+" ( Numerator: Integer; Denominator: Positive ) return Rational is
    begin
        return Normalize( Numerator + Denominator, 1 );
    end;

    function "+" ( X, Y: Rational ) return Rational is
    begin
        return Normalize( X.Numerator * Y.Denominator + Y.Numerator * X.Denominator,  X.Denominator * Y.Denominator);
    end;

    function "+" ( X: Rational; Y: Positive ) return Rational is
    begin
        return Normalize( X.Numerator +  Y * X.Denominator, X.Denominator );
    end;
    function "*" ( Numerator: Integer; Denominator: Positive ) return Rational is
    begin
        return Normalize( Numerator * Denominator, 1 );
